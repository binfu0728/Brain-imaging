function [fov,inCell,outCell,percentage,outPoints,inout] = calculateDensity(cell_mask,oligPoints,width,plotflag)
...
%          inout, the extra column labelling whether each object is inside or outside
    ...
    oligPoints      = oligPoints*0.110; %change from pixel unit to um
    ...
    for k = 1:numCell %find whether points inside or outside through each cell
        ...
        cellb      = cellb*0.110; %change from pixel unit to um
    ...
    fov = size(oligPoints,1) / (0.107*width)^2; %fov density
    if numCell ~= 0
        inCell = size(inPoints,1) / (sum(cell_mask,'all') * (0.107^2));
    else
        inCell = 0;
    end
    outCell = size(outPoints,1) / ((width*width-sum(cell_mask,'all')) * (0.107^2)); %outside cell density
