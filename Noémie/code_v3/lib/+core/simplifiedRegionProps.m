function [pixelIdxList,areas,centroids] = simplifiedRegionProps(BW,imsz)
    ...
    [pixelIdxList,nobj] = bwconncomp_2d(BW,8); %find 8pixel-connected objects, function below
    areas          = zeros(nobj,1); 
    centroids      = zeros(nobj,2);
    for k = 1:nobj
        pil        = pixelIdxList{k}; %indice for each binary object
        [row,col]  = ind2sub(imsz,pil); % turns linear indices found in list pil to lists of rows and cols corresponding values
        areas(k)   = numel(pil); % nb of pixels for each object       
