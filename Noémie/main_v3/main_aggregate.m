...
[filenames,filepaths,z,rsid] = load.loadMeta('test_metadata.csv'); %load metadata to specify where images are saved and how to load them
...
% large objects = non oligomers
% small objects = oligomers
...
for i = 1:length(filenames) %filenames is a nx1 cell array, n being nb of files
    ...
    pgain    = (repmat(gain,[1 1 z(i,2)])); % moved and modified 20230519, last dim has to correspond to zf, may not be 17
    poffset  = (repmat(offset,[1 1 z(i,2)]));
    ...
    % datastore save - should move it to the end to save only 3D images of samples with non oligo results 
    ...
    if saved == 1
        ...
        % so=size(oligomer_result)
        % sno=size(non_oligomer_result)
        % snb=size(numbers)
