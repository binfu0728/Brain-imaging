...
for k = 1:length(filenames)
    ...
    for c = 1:colour
        ...
        % channel = squeeze(c1(:,:,:,1)); %row x col x zaxis
        
        mask            = aggreMask(img1,sigma);
        maskedImage     = mask.*img1;
        ...
        if saveResult
            bgImage         = (1-mask).*img1;
            filledBg        = imfill(bgImage);
            sigImage        = abs(img1-filledBg);
            
            intensity       = zeros(length(aggregatePoints),1);
            avg_inten       = zeros(length(aggregatePoints),1);
            area            = zeros(length(aggregatePoints),1);

            for j = 1:length(aggregatePoints)
                idx             = ind2sub(size(mask),cell2mat(aggregatePoints(j)));
                intensity(j)    = sum(sigImage(idx));
                avg_inten(j)    = intensity(j)/length(idx);
                area(j)         = length(idx)*pixSize^2;
            end
