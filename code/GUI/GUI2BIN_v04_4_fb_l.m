...
settings.plotmask = uicontrol('parent',Analysis_panel,'style','checkbox','position', [.00  .6 1.0 .05],'string','Visualize mask','callback',{@change_selection_method,'analysis'}); 
...
%   Based on oligomer_analysis
    ...
    for i = 1:length(rsid)
        ...
        if settings.extra_metadata.Value
            nums = num2cell(nums);
            nums = [num2cell(nums) repmat(extra_metadata(rsid(i) == extra_metadata.rsid,2:end),size(nums,1),1)];
            inten_all=cell2mat(inten_all);
            inten_all = [num2cell(inten_all) repmat(extra_metadata(rsid(i) == extra_metadata.rsid,2:end),size(inten_all,1),1)];
        end
        nums_z  = [nums_z;nums];
...
%     end
    ...
    if settings.aggre_number.Value
        nums_z.Properties.VariableNames(1:3) = {'small_nums','large_nums','RSID'};
        T1 = nums_z;
        writetable(T1,[filedir(1:num_slash(end-1)),prefix,'_numbers_slice.csv']);
    end

    if settings.small_intens.Value
        inten_t.Properties.VariableNames(1:2) = {'sum_intensity','RSID'};
        T2 = inten_t;
