...
uicontrol('parent',Preprocessing_panel,'style','text','position', [.00  .7 1.0 .15],'string','Select the directory containing round folders. subdir: \Round\Sample\Sample_position');     
...
settings.analysis_group = uibuttongroup('Visible','on','Position',[0 .35 1 .1],'parent',Preprocessing_panel,'SelectionChangedFcn',{@change_selection_method,'preprocessing'}); 
...
%unde development
uicontrol('parent',Preprocessing_panel,'style','text','position',[0  .6 1 .05],'units','normalized','string','denoising','fontsize',10);

% First frame
uicontrol('parent',Preprocessing_panel,'style','text','position',[0  .30 .5 .05],'units','normalized','string','First Frame','fontsize',10);

settings.firstframe = uicontrol('parent',Preprocessing_panel,'style','text','position',[.5  .30 .5 .05],'units','normalized','string','0','fontsize',10);
% Last Frame
uicontrol('parent',Preprocessing_panel,'style','text','position',[0  .25 .5 .05],'units','normalized','string','Last Frame','fontsize',10);
settings.lastframe = uicontrol('parent',Preprocessing_panel,'style','text','position',[.5  .25 .5 .05],'units','normalized','string','0','fontsize',10);
...
% Type of analysis
settings.save_group = uibuttongroup('Visible','on','Position',[0 .20 1 .3],'parent',Analysis_panel); 
...
settings.save_oligomer_table = uicontrol('parent',settings.save_group,'style','checkbox','position',[0  4/6 1 1/6],'string','Oligomer table','value',1,'HorizontalAlignment','left');
settings.save_oligomer_mask = uicontrol('parent',settings.save_group,'style','text','position',[0  3/6 1 1/6],'string','    Oligomer mask','value',0,'HorizontalAlignment','left');
settings.save_Large_aggregate_table = uicontrol('parent',settings.save_group,'style','checkbox','position',[0  2/6 1 1/6],'string','Large-aggregate table','value',1,'HorizontalAlignment','left');
settings.save_Large_aggregate_mask = uicontrol('parent',settings.save_group,'style','text','position',[0  1/6 1 1/6],'string','    Large-aggregate mask','value',0,'HorizontalAlignment','left');
settings.save_cell_table = uicontrol('parent',settings.save_group,'style','checkbox','position',[0  0/6 1 1/6],'string','Cell table','value',0,'HorizontalAlignment','left');
...
filepath_RSID_table = [selection_preprocessing_folder.String(1:find_backslash(end)-1),'_RSID_table.csv'];
...
settings.extra_metadata = uicontrol('parent',Formatting_panel,'style','checkbox','position', [.00  .30 1.0 .05],'string','Include extra metadata','Value',0); 
uicontrol('parent',Formatting_panel,'style','text','position', [.00  .25 1.0 .05],'string','Select RSID table below'); 
selection_RSID_table = uicontrol('parent',Formatting_panel,'style','togglebutton','position', [.00  .2 1.0 .05],'string',filepath_RSID_table,'Value',0,'callback',{@Directory_select,'RSID_table'}); %value 0 and 1 only changes the block colour 
...
%   Based on oligomer_analysis
    filedir = selection_preprocessing_folder.String;
    [~,filepath,zs,rsids] = loadMeta(selection_metadata_file.String);
    ...
    for rsid_i = 1:numel(uniqueRSIDs)
        ...
        for i = rsid_v'
            ...
            [nums_single,~,inten_single] = process.generalAnalysis(smalls,larges,zs(i,:),rsids(i),[],s);
            nums_cell = num2cell(nums_single);
            inten_single = cell2mat(inten_single);
            inten_cell = num2cell(inten_single);
    
            if settings.extra_metadata.Value
                nums_cell = [nums_cell repmat(extra_metadata(rsids(i) == extra_metadata.rsid,2:end),size(nums_cell,1),1)];
    %             inten_num = cell2mat(inten_cell);
                inten_cell = [inten_cell repmat(extra_metadata(rsids(i) == extra_metadata.rsid,2:end),size(inten_cell,1),1)];
            end
            nums_z  = [nums_z;nums_cell];
            inten_t = [inten_t;inten_cell];
    ...
    %         inten_t_wide = inten_cell(1,2);
        ...
        nums_intensity_table_wide{rsid_i} = cell2table(nums_z(1:end,1),'VariableNames',{num2str(rsids(i))});
        nums_area_table_wide{rsid_i}      = cell2table(nums_z(1:end,2),'VariableNames',{num2str(rsids(i))});
        inten_table_wide{rsid_i}          = cell2table(inten_t(1:end,1),'VariableNames',{num2str(rsids(i))});
        nums_z_TOTAL   = [nums_z_TOTAL; nums_z]; %number per slice
        inten_t_TOTAL  = [inten_t_TOTAL; inten_t]; %tmpt holder
    end
    close(f);

    number_of_entries_nums = cell2mat(cellfun(@size,nums_area_table_wide,'UniformOutput',false));
    number_of_entries_nums = number_of_entries_nums(1:2:end);
    maximum_number_of_entries_nums = max(number_of_entries_nums,[],'all');
    for i = 1:numel(nums_area_table_wide)
        nums_intensity_table_wide{i} = [table2array(nums_intensity_table_wide{i}); nan(maximum_number_of_entries_nums-number_of_entries_nums(i),1)];
        nums_area_table_wide{i} = [table2array(nums_area_table_wide{i}); nan(maximum_number_of_entries_nums-number_of_entries_nums(i),1)];
    end
    nums_intensity_table_wide = array2table(cell2mat(nums_intensity_table_wide),'VariableNames',arrayfun(@num2str, uniqueRSIDs, 'UniformOutput', 0)');
    nums_intensity_table_wide = convertvars(nums_intensity_table_wide, @isnumeric, @nanblank);

    nums_area_table_wide = array2table(cell2mat(nums_area_table_wide),'VariableNames',arrayfun(@num2str, uniqueRSIDs, 'UniformOutput', 0)');
    nums_area_table_wide = convertvars(nums_area_table_wide, @isnumeric, @nanblank);
...
% I don't understand why these lines were here. -LEW
...
%     end
    ...
    if settings.aggre_number.Value
        ...
        writetable(nums_z_TOTAL,[filedir(1:num_slash(end-1)),prefix,'_numbers_slice.csv']);

        writetable(nums_intensity_table_wide,[filedir(1:num_slash(end-1)),prefix,'_numbers_brightness_slice_wide.csv']);
        writetable(nums_area_table_wide,[filedir(1:num_slash(end-1)),prefix,'_numbers_area_slice_wide.csv']);
    end

    if settings.small_intens.Value
        inten_t_TOTAL = cell2table(inten_t_TOTAL);
        inten_t_TOTAL.Properties.VariableNames(1:2) = {'sum_intensity','RSID'};
        writetable(inten_t_TOTAL,[filedir(1:num_slash(end-1)),prefix,'_intensity_oligomer.csv']);

        writetable(inten_table_wide,[filedir(1:num_slash(end-1)),prefix,'_intensity_oligomer_wide.csv']);
