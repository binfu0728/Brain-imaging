function [nums_z,inten_z,inten_i,coloc_z] = generalAnalysis(lists,BWs,z,rsid,ref)
...
%          coloc_z, coincidence per slice 
    ...
    for j = 1:length(z(1):z(2))
        nums_z(j,1:length(lists)+length(BWs))     = analyze.numberAnalysis(lists,BWs,z(1)+j-1);
        [inten_z(j,1:length(lists)),inten_t(j,:)] = analyze.intensityAnalysis(lists,z(1)+j-1); 
        if ~isempty(ref)
            s.width          = 2048;
            s.height         = 2048;
            small1           = lists{1}; small1 = load.centroid2BW(round(small1(small1(:,end)==z(1)+j-1,2:3)));
            small2           = lists{2}; small2 = load.centroid2BW(round(small2(small2(:,end)==z(1)+j-1,2:3)));
